...
var testFunctions = {
    ...
    'scale_image' : function(context) {
        var image = new Image();
        image.src = 'canvas_tests_images/rhino.jpg';
        log("outter image function called for ", image.src);
...
function doTest(canvasTD, svgTD, functionTextArea, svgSrcArea) {
    var ctx;
    //log("functionTextArea=",functionTextArea);
    //log("functionTextArea.value=",functionTextArea.value);
    //var testFunction = eval(functionTextArea.value);
    var testFunction = functionTextArea;
...
function addTests() {
    log("getting table");
    var table = MochiKit.DOM.getElement('tests_table');
    var i = 0, start = 2, number = 1;
    for (var test in testFunctions) {
        if (i>=start && i-start<number) {
            ...
            var tr = TR(null, TD(null, ""+i+": "+test, BR(null), IMG({'src':'canvas_tests_images/'+test+'.png'}) ), 
                              canvasTD=TD(null), 
                              TD(null, functionTextArea=TEXTAREA({'rows':"14", 'cols':"40", 'wrap':"off"}, plotSrc),
                                       BR(null),
            ...
            //doTest(canvasTD, svgTD, functionTextArea, svgSrcArea);
            doTest(canvasTD, svgTD, testFunctions[test], svgSrcArea);
        }
        i++;
    }
    var compositeTypes = [
