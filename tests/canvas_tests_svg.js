...
var testFunctions = {
    ...
    'interactive_gears': function (c) {
        ...
        var thickness = 20;  // Thickness of ZW Piece

        var height = Math.sqrt(diagonal*diagonal-bottom*bottom/4)
        var r = diagonal/2  // Radius of the gear (between the outer and inner)
        ...
        var r2 = diagonal-thickness*0.7
        
        var medium = 'rgba(0, 0, 0, 0.6)'
        ...
        */
        ...
        c.stroke()
        c.beginPath()
        //c.arc(center-bottom/2, center+height, diagonal, 0, -2*a, true)  // Through Center
        c.arc(center-bottom/2, center+height, r2, 0, -2*a, true)  // Through Center
        c.stroke()
        ...
        var rotateGears = function(c,a,g1,g2) {
           // Rotate the gears and the drawn lines.
           var rotate = 0.0 * 2*a  // Rotate between 0.0 and 2*a
           log("rotate=", rotate/Math.PI*180)  // Servo Maximum is 
           c.svg.rotate(g1 ,rotate/Math.PI*180)
           c.svg.rotate(g2 ,-rotate/Math.PI*180)
        }
        
        var rotateCallback = function(c,a,g1,g2,center_circ,e,drag) {
           var ox = drag.coords.x
           var oy = drag.coords.y
           var mx = e.mouse().client.x
           var my = e.mouse().client.y
           
           var pos = MochiKit.DOM.elementPosition(c.svg.htmlElement.parentNode)
           
           // Find the old mouse angle
           var x0 = ox-pos.x-center
           var y0 = oy-pos.y-center
           var th0 = Math.atan2(y0, x0)
           
           // Find the current mouse angle
           var dx = mx-pos.x-center
           var dy = my-pos.y-center
           var th = Math.atan2(dy, dx)
           
           drag.last_rotate = SVGKit.firstNonNull(drag.last_rotate, 0)
           
           
           var new_rotate =  th - th0 // 1/6 * 2*a  // Rotate between 0.0 and 2*a
           var rotate = -drag.last_rotate + new_rotate
           drag.last_rotate = new_rotate
           
           /*
           log(pos, "Mouse: ",
               drag.coords, e.mouse().client, 
               drag.page, e.mouse().page, 
               "th0:",th0/Math.PI*180, "th:",th/Math.PI*180,
               "new:", new_rotate/Math.PI*180, "rotate:",rotate/Math.PI*180)
           */
           
           c.svg.rotate(g1 ,rotate/Math.PI*180)
           c.svg.rotate(g2 ,-rotate/Math.PI*180)
           //g1.setAttribute('transform', 'rotate('+(new_rotate/Math.PI*180)+')');
           //g1.setAttribute('transform', 'rotate('+(-new_rotate/Math.PI*180)+')');
        }
        
        c.svg.enableDrag(g1, partial(rotateCallback, c,a,g1,g2,center_circ), false)
    }
